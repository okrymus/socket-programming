...
def P2P(serverName,serverPort,filename):
    ...
    while(False in lst):
        if (IsActivePeers.count('inactive') > 0):
            ...
            if len(peers_IP) < MAX_PEERS and iterate < 15:
    ...
    #     n = random.sample([i for i, e in enumerate(lst) if e == False],len([i for i, e in enumerate(lst) if e == False]))[0]
    #     # lst[n] = CS('128.119.245.41',42602,"redsox.jpg:"+str(n))
    #     print(n)
    #     d = DownThread('128.119.245.41',5104,"redsox.jpg:"+str(n),n)
    #     threads.append(d)
